# 08 - Bundles

<!-- FRKP-NAV-START -->
[Financial Risk Knowledge Platform](../README.md) > [Home](../README.md) > [08_Bundles](README.md)

---

## Navigation

### Parent Repository

[Parent Repository](../README.md)

### Sibling Layers

- [Reference Library](../01_Reference_Library/README.md)
- [Knowledge Base](../02_Knowledge_Base/README.md)
- [Analysis](../03_Analysis/README.md)
- [Mathematical Foundation](../05_Mathematical_Foundation/README.md)
- [Formula Catalog](../04_Formula_Catalog/README.md)
- [Implementation Guide](../06_Implementation_Guide/README.md)
- [Architecture Guide](../07_Architecture/README.md)

### Bundle List

- [BUNDLE-001](BUNDLE-001_BASEL_III_REVIEW.md)
- [BUNDLE-002](BUNDLE-002_FRTB_REVIEW.md)
- [BUNDLE-003](BUNDLE-003_IFRS9_REVIEW.md)
- [BUNDLE-004](BUNDLE-004_SA_CCR_REVIEW.md)
- [BUNDLE-005](BUNDLE-005_CVA_REVIEW.md)
- [BUNDLE-006](BUNDLE-006_MARKET_RISK_STANDARDIZED_APPROACH_REVIEW.md)
- [BUNDLE-007](BUNDLE-007_OPERATIONAL_RISK_REVIEW.md)



### Related Standards

- [FRKP-DOC-100](../00_Project_Management/Governance/FRKP-DOC-100_MASTER_DOCUMENT_INDEX.md)
- [FRKP-DOC-001](../00_Project_Management/Governance/Standards/FRKP-DOC-001_DOCUMENT_STANDARD.md)
- [FRKP-ID-001](../00_Project_Management/Governance/Standards/FRKP-ID-001_DOCUMENT_IDENTIFIER_STANDARD.md)
- [FRKP-BUNDLE-001](../00_Project_Management/Governance/Standards/FRKP-BUNDLE-001_BUNDLE_STANDARD.md)
- [FRKP-FORM-001](../00_Project_Management/Governance/Standards/FRKP-FORM-001_FORMULA_STANDARD.md)
<!-- FRKP-NAV-END -->

## Purpose
Bundle review documents that certify the completion, traceability, and governance status of each FRKP delivery bundle.

## Scope
- Bundle review and closure records
- Bundle deliverable checklists
- Cross-layer traceability summaries
- Freeze and handoff readiness evidence

## Bundle review documents
- `BUNDLE-001_BASEL_III_REVIEW.md`
- `BUNDLE-002_FRTB_REVIEW.md`
- `BUNDLE-003_IFRS9_REVIEW.md`
- `BUNDLE-004_SA_CCR_REVIEW.md`
- `BUNDLE-005_CVA_REVIEW.md`
- `BUNDLE-006_MARKET_RISK_STANDARDIZED_APPROACH_REVIEW.md`
- `BUNDLE-007_OPERATIONAL_RISK_REVIEW.md`

## Relationship with other folders
| Folder | Relationship |
|---|---|
| 01_Reference_Library | Bundle reviews verify reference deliverables. |
| 02_Knowledge_Base | Bundle reviews verify knowledge deliverables. |
| 03_Analysis | Bundle reviews verify analysis deliverables. |
| 04_Formula_Catalog | Bundle reviews verify formula deliverables. |
| 05_Mathematical_Foundation | Bundle reviews record optional mathematical foundation deliverables when required. |
| 06_Implementation_Guide | Bundle reviews verify implementation deliverables. |
| 07_Architecture | Bundle reviews verify architecture deliverables. |